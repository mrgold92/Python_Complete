...
for linea in (file.readlines()):
    ...
    if(data[0].isdigit() == True):
        ...
        cliente.FechaAlta = datetime.strptime(
            data[6].strip(), "%d/%m/%Y").date()
...
resultado = list(filter(lambda x: x.Edad == e and x.Pais ==
                 p and x.Genero == g, clientes))
...
for item in resultado:
    contador += 1
    file2.write(
        f"{contador},{item.Nombre},{item.Apellidos},{item.Genero},{item.Pais},{item.Edad},{item.FechaAlta.strftime('%d/%m/%Y')}\n")
