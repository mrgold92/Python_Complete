...
headers = {'X-ClientId': 'escribe tu client ID',
           'passKey': 'escribe tu pass key'}
token = None
...
if(token != None):
    ...
    response = requests.post(url['StopInfo'].replace(
        '<stopId>', stopId), headers=headers, data=json.dumps(data))
    if(response.status_code == 200):
